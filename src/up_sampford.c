...
static void sampford_srs(int N, int m, int *out) {
  ...
  for (int i = 0; i < m; i++) {
    ...
    if (j >= N)
      j = N - 1; /* guard a hypothetical rounded U == 1 */
 ...
 * with odds p_i/(1-p_i).  Exactly the Sampford design.
 */
static void sampford_nonrejective(const double *p, const double *odds, int N,
                                  int m, int *out) {
  if (m == 1) {
  ...
  double *f_ex = (double *)R_alloc(CPS_TABLE_SIZE(N - 1, r), sizeof(double));
  ...
  if (selected != r) {
    error(
        "Sampford conditional-Poisson fallback produced %d units, expected %d",
        selected, r);
  }
  for (int i = 0; i < r; i++)
    out[i + 1] = map[draw_ex[i]];
}
...
static void sampford_draw_work(const double *p, int N, int m, int *out,
  ...
  if (!(odds_total > 0.0) || !R_FINITE(odds_total)) {
    error("Sampford odds are non-finite; inclusion probabilities are too close "
          "to 1");
  }
