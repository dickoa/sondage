...
`probabilities` places the method in the first-order probability taxonomy. Use `"exact"` when the true first-order inclusion probabilities, or expected hits for `"wr"`, equal the `pik` passed to `sample_fn`, as they do for Sampford and the cube method. Use `"approximate"` when the method honors `pik` to a documented approximation, as Pareto and sequential Poisson order sampling do. Use `"unknown"`, the default, when `pik` only steers the selection. Successive sampling with `sample.int(prob = pik)` belongs to this last tier because its true inclusion probabilities differ from `pik`. The same draw with replacement does honor expected hits, so a multinomial-style `"wr"` method declares `"exact"`.
