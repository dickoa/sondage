...
### High-entropy approximation for joint probabilities

Sampford's design is a high-entropy design, so the high-entropy (HE) approximation of @brewer2003 applies:
...
We can write this directly in `R`:

```{r he-joint-fn}
he_joint <- function(pik, sample_idx = NULL, ...) {
  n   <- sum(pik)
  nm1 <- n - 1
  S2  <- sum(pik^2)

  ck <- nm1 / (n - (2 * n - 1) / nm1 * pik + S2 / nm1)

  pikl <- outer(pik, pik) * (outer(ck, ck, "+") / 2)
  pikl <- pmin(pikl, outer(pik, pik, pmin))   # upper bound
  pikl[pikl < 0] <- 0                          # lower bound
  diag(pikl) <- pik
...
This is the same formula used internally by `sondage` for Brewer, SPS, and Pareto methods.
...
The HE approximation is close because Sampford's design is itself
maximum-entropy.
...
`joint_expected_hits()`, and `sampling_cov()`. Three common
strategies:

1. **Exact formula.** When the design has a known closed form
   (e.g., `UPtillepi2` above).

2. **High-entropy approximation.** When the design is close to
   maximum entropy but no exact formula exists. The HE formula
   shown above is a good default.

3. **Monte Carlo estimation.** Resample $B$ times and estimate
