...
register_method(
  ...
  exact_chance = NULL,      # does the method honor pik as its chance target? (optional)
...
`exact_chance` declares whether the method honors `pik` as its first-order chance target: `TRUE` when the realized inclusion probabilities (or expected hits, for `"wr"`) equal the `pik` passed to `sample_fn`, to the same standard as the built-ins; `FALSE` when `pik` only steers the selection, as in successive sampling with `sample.int(prob = pik)`, whose realized inclusion probabilities differ from `pik` (the same draw *with* replacement does honor expected hits, so a multinomial-style `"wr"` method can declare `TRUE`). The flag matters to packages that weight estimation by `1/pik`: for a `FALSE` method those design weights are systematically biased, not merely noisy, so such packages may read the flag through `method_spec()` and refuse the method for weighted estimation rather than assert chances the design never had. Sampling through sondage itself is unaffected — the declaration describes the method, it never disables it. Like `variance_family`, it is an assertion the author is responsible for — the simulation below shows how to check it — and `NULL` (the default) means undeclared.
