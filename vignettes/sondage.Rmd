...
For high-entropy methods (`brewer`, `sps`, `pareto`, `cube`), `poisson`, `bernoulli`, `srs`, and `multinomial`, the submatrix is computed directly without allocating the N x N matrix, so arbitrarily large populations are supported. For `cps`, `systematic`, and `chromy`, the full matrix is still needed internally, so the N > 10,000 guard remains. The `sampling_cov()` generic also accepts `sampled_only`:
